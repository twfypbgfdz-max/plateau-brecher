// Kalorien-App Sync v0.4.3
// Lokale Vorbereitung fuer spaeteren Web-App-Deploy. Keine Sheet-IDs, Tokens oder Trigger.
// Fix: upsertRowsByKey_ normalisiert Sheet-Werte typsicher (Date-Objekt → YYYY-MM-DD).
//      First-wins-Logik verhindert Duplikat-Lücken bei bereits mehrfach vorhandenen Keys.
...
// Sheets gibt Datumszellen als JS-Date-Objekt zurück — das wird hier zu YYYY-MM-DD.
...
function upsertRowsByKey_(sheet, headers, rows, keyName) {
  ...
  // Key-Map aufbauen: first-wins bei Duplikaten im Sheet.
  // normalizeSheetKey_ wandelt Date-Objekte (Sheets-Autokonvertierung) in YYYY-MM-DD um.
  const existingRowByKey = {};

  existingValues.forEach((row, index) => {
    const key = normalizeSheetKey_(row[keyIndex]);
    if (!key) return;
    if (existingRowByKey[key] !== undefined) return; // erste Zeile gewinnt, Duplikate ignorieren
